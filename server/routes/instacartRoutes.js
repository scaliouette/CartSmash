...
async function createRecipeFromCartItems(items, retailerId, zipCode, metadata) {
  try {
    ...
    // Convert cart items to recipe ingredients
    ...
    const recipePayload = {
      title,
      author: 'CartSmash Checkout',
      servings: 4,
      image_url: `https://via.placeholder.com/500x500/4CAF50/white?text=${encodeURIComponent(title)}`,
      instructions,
      ingredients,
      external_reference_id: `cartsmash_checkout_${Date.now()}`,
      content_creator_credit_info: 'Generated by CartSmash Checkout',
      expires_in: 30,
    ...
    if (validateApiKeys()) {
      ...
      console.log(`✅ Recipe created for checkout: ${result.instacartUrl}`);
      return result;
    } else {
      // Mock recipe URL for development
