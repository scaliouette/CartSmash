// client/src/components/ParsedResultsDisplay.js - BRONCOS THEMED VERSION
...
function ParsedResultsDisplay({ items, onItemsChange, currentUser, parsingStats }) {
  ...
  const getConfidenceColor = (confidence) => {
    if (confidence >= 0.8) return '#FB4F14'; // Broncos Orange for high
    if (confidence >= 0.6) return '#FFA500'; // Light orange for medium
    return '#002244'; // Broncos Navy for low
  };
...
const styles = {
  ...
  batchOperationsBar: {
    background: 'linear-gradient(135deg, #FFF5F2, #FFE5D9)',
    padding: '12px 16px',
    borderRadius: '8px',
    marginBottom: '16px',
    display: 'flex',
